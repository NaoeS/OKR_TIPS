- 目次
  ...
  * [Rails 4 ブロンズ試験 体験記](#rails-bronze-memo)
...
## 感想
- ネットに落ちている体験記にも書かれているが、予想していたものより難しい
- 問題の複雑さから来る難しさというよりは、これはどっちだろう・・・のうろ覚えによって迷うことが多かった
  * 正しいことをどれだけ覚えているかが大事
- CTC の模擬問題を満点にするだけでは確かに不十分。ガイドを読み通すことも重要
  * 模擬で誤った問題を反芻するというよりは、その問題に該当する項目をガイドで復習すると良い
- 各問題のセクションで感じたこと
  * Rails 内部の初期化手順や設定ファイルの場所は抑えておく
  * ERB のコメント手段、出力される or されないの区別はできるようにしておく
  * バリデーションのオプションに関する問題は問題文とにらめっこして正しい方を選択できるようにする
  * Active Record のコールバック順は覚えておく
  * Action View のメソッドが何を出力するかは実際に試すのが吉。オプションについても学ぶ必要がある
    + パーシャルの描画方法などは区別できるように
  * `resources`, `resource` はどんなルーティングを設定するのか、どんなヘルパーを使えるようになるか覚えておく
  * 拡張メソッドはすべて覚えるのは難しいが、時間が許す限り目を通しておく
  * 多言語化の呼び出し方は知らないと戸惑う。流し見してたので大分厳しかった
